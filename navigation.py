...
class Navigation_Env(BaseEnv):
    ...
    def step(self, action):
        ...
        for idx, defender in enumerate(self.defender_list):
            ...
            if defender.active:
                reward, collision, caught = self.defender_reward(idx, state, next_state, self.target[0])  # self.target[idx]
                rewards.append(reward)
                if caught:
                    # self.update_attacker(idx=idx)
    ...
    def collision_detection(self, state, obstacle_type: str = 'obstacle', next_state: list = None, target: tuple = None, idx=None):
        ...
        elif obstacle_type =='protector':
            distances = np.linalg.norm(self.get_state(agent_type=obstacle_type)[:, :2] - state[:2], axis=1)
            return distances <= self.defender_config.collision_radius
        elif obstacle_type == 'target':
            if np.linalg.norm([target[0] - state[0], target[1] - state[1]]) <= self.env_config.goal_range:
                arrived = True
                ray_indices = bresenham_line(round(state[0]), round(state[1]), round(target[0]), round(target[1]))
                for k in range(len(ray_indices)):
                    index = ray_indices[k]
                    if  not self.occupied_map.is_unoccupied((index[0], index[1]), self.occupied_map.grid_map):
                        arrived = False
                        break
            else:
                arrived = False
    ...
    def demon(self):
        ...
        attacker_state = self.get_state(agent_type='attacker').astype(np.int64)
        (width, height) = self.occupied_map.boundaries
        self.occupied_map.set_moving_obstacle(pos=defender_state)
